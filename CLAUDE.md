...
- **Hooks**: mutating commands fire `pre-<cmd>` / `post-<cmd>` hooks via `internal/hooks`. These are git-hop's own lifecycle hooks, NOT git's — they never live in `.git/hooks/`. `FindHookFile` resolves repo (`<worktree>/.git-hop/hooks/`, plus a parent-dir walk) → hopspace (`hop.HopspaceHooksDir`, i.e. `<hopspace>/hooks/`, then the pre-layout `$GIT_HOP_DATA_HOME/github.com/<org>/<repo>/hooks/`) → global (`$XDG_CONFIG_HOME/git-hop/hooks/`). Exception: `pre-clone` has no repo level (the repo does not exist yet), so it resolves only at hopspace and global. `ValidHookNames` in `internal/hooks/runner.go` is the authority on which names exist; see `docs/hooks.md` for which are actually dispatched and at which levels. The committed-hook mirror writes the hopspace hooks dir under that hopspace's `hop.json.lock`, the lock doctor --fix moves it under: every hook is decided (prompts included) and the dir resolved first, and only the writes run under the lock (`internal/hooks/install_write.go`). Hubs of one repo can resolve `hop.dataLayout` differently; doctor --fix moves no data-home dir while they do, and the mirror warns (one check: `hop.ResolveHubLayouts` / `AgreedHopspace` / `LayoutAlignmentHint`).
