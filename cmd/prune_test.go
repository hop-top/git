...
func TestPruneOrphanedWorktrees(t *testing.T) {
	...
	pruned := len(pruneOrphanedWorktrees(fs, mocks.NewMockGit(), st, false, nil))
...
func TestRunPrune_DryRun(t *testing.T) {
	...
	wt, hubs := runPruneFS(fs, mocks.NewMockGit(), loaded, true /* dryRun */, nil)
...
func TestRunPrune_Apply(t *testing.T) {
	...
	wt, _ := runPruneFS(fs, mocks.NewMockGit(), loaded, false /* dryRun */, nil)
	assert.Len(t, wt, 1)
...
func TestPruneOrphanedHubs(t *testing.T) {
	...
	// Prune orphaned hubs
	pruned := len(pruneOrphanedHubs(fs, st, false, nil))
