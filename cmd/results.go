...
type envGCRecord struct {
	...
	Key      string `json:"key" yaml:"key" table:"key" jsonschema:"description=Dependency key (<deps dir>.<lockfile hash>); for a store an earlier release left under the data home, its path there (<name>/deps)"`
