...
func TestDoctorFixStateIssues_ClearsHopJSONRows(t *testing.T) {
	...
	fixed := fixStateIssues(fs, mocks.NewMockGit(), st, hubPath, doctorOpts{fix: true}, &doctorReport{})
...
func TestDoctorFixStateIssues_BacksUpHopJSON(t *testing.T) {
	...
	require.Equal(t, 1, fixStateIssues(fs, mocks.NewMockGit(), st, hubPath, doctorOpts{fix: true}, &doctorReport{}))
...
func TestDoctorFixStateIssues_NoOpWhenClean(t *testing.T) {
	...
	assert.Equal(t, 0, fixStateIssues(fs, mocks.NewMockGit(), st, hubPath, doctorOpts{fix: true}, &doctorReport{}))
	assert.ElementsMatch(t, []string{"main"}, hubBranchKeys(t, fs, hubPath))
...
func TestDoctorFixStateIssues_ScopedToCurrentHub(t *testing.T) {
	...
	assert.Equal(t, 1, fixStateIssues(fs, mocks.NewMockGit(), st, hubPath, doctorOpts{fix: true}, &doctorReport{}))
...
func TestDoctorFixStateIssues_NoHubFallsBackToState(t *testing.T) {
	...
	assert.Equal(t, 0, fixStateIssues(fs, mocks.NewMockGit(), st, "", doctorOpts{fix: true}, &doctorReport{}))
