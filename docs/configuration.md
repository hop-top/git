...
in an entry without a `path`. Removing a hub drops its entries, as do
`git hop prune` and `git hop doctor --fix` for a hub whose directory is
gone; `git hop doctor` warns about entries of a hub gone some other way,
and `--fix` drops them.
