...
from the worktree looking for one. Editor and tool configs that are
per-repo rather than per-branch (`.claude/`, and the equivalents for
whatever tooling you use) follow the same pattern.
