...
git hop add <branch> --no-fetch       # skip refreshing origin before resolving the start-point
...
```bash
git hop doctor                        # diagnose paths, hubs, hopspaces, orphans (exit 1 on issues)
