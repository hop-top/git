...
Earlier releases mirrored hooks to `github.com/<org>/<repo>/hooks/` under the data home. Those hooks keep firing. `git hop doctor` warns about each such directory, and `git hop doctor --fix` moves it to the `hop.dataLayout` location when nothing is there yet (`--fix --dry-run` previews the move). With hooks at both locations it moves nothing and names both, so you can merge them by hand. When hubs of the repository resolve `hop.dataLayout` differently (a hub's own value overriding `--global`), it moves nothing either, and its `hint:` names each hub's value and how to align them.
...
Every hook is decided first, prompts included, and the hooks are then written together while holding the `hop.json.lock` of the data-home hopspace the hooks directory belongs to (`$GIT_HOP_DATA_HOME/<org>/<repo>/hop.json.lock` by default), never while a prompt waits. `git hop doctor --fix` moves hooks directories under the same lock, so a move waits for the hooks being written and carries them, and a mirror waits for a move in progress. A mirror that finds the directory moved away once it holds the lock writes nothing there: it looks up where `hop.dataLayout` now puts the directory and writes there. When that is still the directory just moved away, the move followed another hub's `hop.dataLayout`: the mirror writes nothing, with a `warning:` and a `hint:` to align the hubs and mirror again. An empty hopspace directory that taking its lock recreated at the old path is removed again. Before any of this, a mirror whose worktree resolves `hop.dataLayout` to another hopspace than a hub of the same repository recorded in state warns, naming each hub's value and how to align them, and mirrors where its own worktree resolves: that is where its hook lookup reads. A hook that appeared meanwhile with different content is kept, with a `warning:`, unless `--hooks-overwrite` or a prompt answered for it allows replacing it.
